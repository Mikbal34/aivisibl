...
function computeLLMScores(
  ...
  // 4. Mentions (sadece raw count, yüzdelik yok)
  let mentions_raw = 0
  ...
  llm_runs.forEach(run => {
    ...
    mentions_raw += countDomainMentions(run.response_text, config.domain_variants)

    // Count source mentions
    run.sources.forEach(source => {
      if (matchesDomainVariant(source.url, config.domain_variants)) {
        mentions_raw++
      }
    })
  })
